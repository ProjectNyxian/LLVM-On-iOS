...
swift-project:
	...
	cd "$(SWIFT_REPO_DIR)" && git checkout "$(SWIFT_BRANCH)"; \
	cd "$(SWIFT_REPO_DIR)" && utils/update-checkout --clone --tag "$(SWIFT_BRANCH)" --source-root "$(SWIFT_PROJECT)"
